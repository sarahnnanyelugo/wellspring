import "./_footer.scss";
import { Link } from "react-router-dom";
import Logo from "../../assets/Logo.png";
import { SearchBar } from "../SearchBar/SearchBar";
import Educare from "../../assets/educare.png";
import { MessageBox } from "../MessageBox/MessageBox";

export const Footer = () => {
  return (
    <>
      <div className="footer-container col-md-12 flexy">
        <div className="col-md-5">
          <div className="col-md-12 links-container flexy">
            <div className="col-md-6">&nbsp;</div>
            <div className="col-md-6 flexy">
              <ul className="list-unstyled">
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    ABOUT
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    ADMISSIONS
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    ATHLETICS
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    BLOGS
                  </Link>
                </li>
              </ul>
              <div className="col-md-2">&nbsp;</div>
              <ul className="list-unstyled">
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    ACADEMICS
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    ARTS
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    GIVING
                  </Link>
                </li>
                <li>
                  <Link to={"#"} className="footer-links-alteration">
                    STORE
                  </Link>
                </li>
              </ul>
            </div>
          </div>
          <div className="col-md-12 address-container flexy">
            <div className="col-md-6">&nbsp;</div>{" "}
            <div className="col-md-6">
              <p>25B Somide Odujinrin Street Omole Phase 2, Ikeja, Lagos.</p>
              <p>
                <Link to={"#"} className="link-alteration">
                  (+234) 803 395 4138
                </Link>
              </p>
              <p>
                <Link to={"#"} className="link-alteration">
                  info@wellspringcollege.org
                </Link>
              </p>
            </div>
          </div>
        </div>
        <div className="footer-logo-container col-md-2">
          {" "}
          <img className="col-md-12 " src={Logo} alt="First slide" />
        </div>
        <div className=" col-md-5">
          <div className="footer-handles-container col-md-12 flexy flexyM">
            <div className="col-md-6">
              <ul className="list-unstyled list-inline">
                <li className="list-inline-item ">
                  <Link to={"#"} className="link-alteration">
                    Alumni
                  </Link>
                </li>{" "}
                <li className="list-inline-item ">
                  <Link to={"#"} className="link-alteration">
                    Careers
                  </Link>
                </li>{" "}
                <li className="list-inline-item ">
                  <Link to={"#"} className="link-alteration">
                    Volunteer
                  </Link>
                </li>{" "}
                <li className="list-inline-item ">
                  <Link to={"#"} className="link-alteration">
                    Family Portal
                  </Link>
                </li>
              </ul>

              <ul className="list-inline list-unstyled">
                <li className="list-inline-item">
                  <Link className="link-icon-alteration">
                    <i class="icofont-instagram"></i>
                  </Link>
                </li>
                <li className="list-inline-item">
                  <Link className="link-icon-alteration">
                    {" "}
                    <i class="icofont-twitter"></i>
                  </Link>
                </li>
                <li className="list-inline-item">
                  <Link className="link-icon-alteration">
                    <i class="icofont-facebook"></i>
                  </Link>
                </li>
                <li className="list-inline-item">
                  <Link className="link-icon-alteration">
                    <i class="icofont-brand-youtube"></i>
                  </Link>
                </li>
              </ul>
              <SearchBar />
            </div>

            <div className="col-md-6">&nbsp;</div>
          </div>

          <div className="sighoff-container col-md-12 flexy">
            <div className="col-md-1">&nbsp;</div>
            <div>
              <p>© Copyright 2022 Wellspring College.</p>
              <p>
                <p>All Rights Reserved.</p>
                <Link to={"#"} className="link-alteration">
                  Cookies Policy .
                </Link>{" "}
                <Link className="link-alteration"> Privacy Policy</Link>
              </p>
              <div className="col-md-12 educare flexy flexyM">
                <p>Powered by</p>
                <a
                  href="https://wellspring.educare.school/admission-form"
                  target="_blank"
                >
                  {" "}
                  <img className="col-md-12 " src={Educare} alt="Scholar" />
                </a>
              </div>
            </div>
            <div className="col-md-5 message-box flexy">
           <div className="col-md-4">&nbsp;</div>
              <MessageBox />
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
